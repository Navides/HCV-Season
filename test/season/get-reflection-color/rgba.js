...
describe("Season.getReflectionColor - rgba", () => {
    ...
    it("1: 第1引数がRGBAの色表現の場合は、調整した色のRGBA表現が返却される", () => {
        ...
        for(const combination of combinations) {
            ...
            // 結果を検証
            expect(result).toBe(`rgba(12,30,47,${ExpressionConverter.getPercent(alpha)})`);
        }
    });
    ...
    it("2: 第1引数がRGBAの色表現を含む場合は、調整した色のRGBA表現に置換した内容が返却される", () => {
        ...
        for(const combination of combinations) {
            ...
            // 結果を検証
            expect(result).toBe(`linear-gradient(rgba(14,32,50,${ExpressionConverter.getPercent(alpha)}), rgba(60,78,96,${ExpressionConverter.getPercent(alpha)}))`);
        }
    });
});